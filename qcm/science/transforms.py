...
def compute(
    ...
    if q.kind == "mpe":
        ...
        #
        # The charge column is only present because the mpe Quantity lists
        # "charge" in its ``sources`` (theme.py) and the select at the top of
        # this function kept it — guard that hidden contract explicitly.
        if "charge" not in out.columns:
            raise ValueError(
                "MPE needs the charge column; the 'mpe' quantity must keep "
                "'charge' in its sources (see theme.QUANTITIES)."
            )
...
def sauerbrey_check(summary: pl.DataFrame) -> dict | None:
    ...
    from .quantities import SAUERBREY_RATIO_MAX, SAUERBREY_SPREAD_MAX_PCT
