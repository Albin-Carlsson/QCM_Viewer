"""Science constants, experiment parameters, and the quantity/axis registries.

The UI-free half of what used to be ``qcm.viz.theme``: everything here is plain
Python/dataclasses with no rendering imports, so the storage layer (notebook
export), the science transforms, and the UI all read the same definitions.
Display policy (colours, plot heights, default visible overtones) stays in
``qcm.viz``.
"""
...
__all__ = [
    "DISSIPATION_SCALE", "SAUERBREY_CONSTANT", "FARADAY_CONSTANT",
    "ELECTRODE_DISC_RADIUS_CM", "ELECTRODE_AREA_CM2",
    "DEFAULT_MOLAR_MASS_G_PER_MOL", "DEFAULT_VALENCY",
    "NG_PER_CM2_TO_G", "MM_PER_CM",
    "CHARGE_EPS_C", "FREQ_EPS_HZ", "AREA_MIN_CM2",
    "CP_CURRENT_COV_THRESHOLD",
    "MPE_CLIP_LO_DEFAULT", "MPE_CLIP_HI_DEFAULT",
    "MPE_SMOOTH_WINDOW_DEFAULT", "MPE_SAVGOL_POLYORDER",
    "DESPIKE_WINDOW_DEFAULT", "DESPIKE_THRESHOLD_SIGMA", "MAD_TO_SIGMA",
    "ALIGNMENT_MAX_LAG_S",
    "RHO_QUARTZ_G_CM3", "MU_QUARTZ_G_CM_S2", "DEFAULT_CRYSTAL_F0_MHZ",
    "SAUERBREY_RATIO_MAX", "SAUERBREY_SPREAD_MAX_PCT",
    "sensitivity_from_f0", "area_to_diameter_mm", "potential_axis_label",
    "ExperimentParams", "DEFAULT_PARAMS",
    "Quantity", "QUANTITIES", "quantity",
    "Axis", "AXES", "axis", "ELAPSED_COLUMN",
]
...
QUANTITIES: dict[str, Quantity] = {
    ...
    for q in [
        ...
        # MPE keeps "charge" in sources deliberately: the science transform reads
        # the charge column it carries through compute() (see transforms.compute).
