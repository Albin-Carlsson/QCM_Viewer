...
class ViewerControls:
    ...
    def _build_widgets(self) -> None:
        group_options = {f"n={self.info.orders.get(g, 1)} · group {g}": str(g) for g in self.info.groups}
        ...
        self.sweep_mode = pn.widgets.RadioButtonGroup(
            name="Show",
            options=["selected overtones", "single group"],
    ...
    def sweep_inputs(self) -> tuple:
        return (self.sequence.param.value_throttled, self.group_select, self.group_for_single, self.sweep_mode)
    ...
    def zero_reference_controls(self):
        return pn.Column(
            pn.pane.Markdown(
                "<small><b>Zero/reference range</b> is the quiet interval that should count as zero for Δ-values. "
            ...
            self.baseline_range,
            pn.Row(self.use_selection_as_baseline, self.revert_baseline, sizing_mode="stretch_width"),
            pn.pane.Markdown(
                "<small>Press <b>Set zero = current range</b> after selecting a stable reference period. "
                "Then move the current range anywhere else to inspect the experiment relative to that zero.</small>",
                sizing_mode="stretch_width",
            ),
            sizing_mode="stretch_width",
            css_classes=["plot-controls"],
    ...
    def window_controls(self, include_zero_reference: bool = True):
        ...
        return pn.Column(*items, sizing_mode="stretch_width", css_classes=["plot-controls"])
